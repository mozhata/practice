...
const usage = `
 ...
 Usage:
  	...
  	-e     files not wath, use relative path
...
var (
	...
	exclude    = flag.String("e", "", "files not watch")
...
func main() {
	...
	if *exclude != "" {
		for _, f := range strings.Split(*exclude, ",") {
			excludeFolders = append(excludeFolders, filepath.Join(wd, f))
		}
	}
	...
	w.watcher(recursivePath(*recursive, append(flag.Args(), wd), excludeFolders))
...
func recursivePath(recursive bool, paths, excludeFolders []string) []string {
	if !recursive {
		return paths
	}
