...
// catalogURL := "http://www.biquge.cm/11/11856/"
...
func gaowu() {
	var (
		host = "https://www.biqugex.com"
		// novelNum = "book_83682"
		size = 200
		from = 350
	)

	catalogURL := "https://www.biqugex.com/book_83682/"
	links, err := getChaperListGW(from, size, catalogURL)
	if err != nil {
		glog.Fatalf("getChapterList failed: %v", err)
	}
	// download
	for i, l := range links {
		if strings.HasPrefix(l.Href, "/book_83682/") {
			filename := strings.TrimPrefix(l.Href, "/book_83682/")
			if filename != "" {
				chapterURI := host + l.Href
				downloadFileDec(chapterURI, filename)
				glog.Infof("\n\nthe %dst chap %s downloaded", i, l.Title)
			}
		}
	}

	novelFile := fmt.Sprintf("quanqiugaowu_%d_%d.txt", from, size)
	// parse
	for i, l := range links {
		glog.Infof("parsing the %dst chapter: %s", i, l.Title)
		if strings.HasPrefix(l.Href, "/book_83682/") {
...
func getChaperListGW(from, size int, catalogURL string) ([]chapterLink, error) {
	doc, err := goquery.NewDocument(catalogURL)
	if err != nil {
		panic(fmt.Sprintf("failed to catalogURL. err: %s", err))
	}
	...
	result := make([]chapterLink, 0, size)
	chapList := doc.Find(".listmain >dl > dd > a")
	for i := from; i < chapList.Length(); i++ {
		if i > from+size {
			break
		}
